"""Retake all Blastoise CSPs in the 'blastoise' pose.

Blastoise is a Bowser reskin (PlyKoopa5K skeleton; its DATs reuse PlKp* names).
The 'blastoise' pose (custom_poses/Bowser/blastoise.yml) is a SYMBOL pose
(animSymbol: PlyKoopa5K_Share_ACTION_SpecialSStart_figatree, frame 11). The
symbol lives in Bowser's vanilla AJ, so we render each costume DAT with the
pose yml + PlKpAJ.dat. Generates SD (scale 1, 136x188) + HD (scale 4,
544x752) and writes them to every CSP location (root csp_<i>, costumes
sidecars + zip, fighter.zip inner zip). Stocks (stc.png) are untouched.
Run from modellab/.
"""
import io
import subprocess
...
VAULT = ROOT / "storage/custom_characters/blastoise"
EXE = ROOT / "utility/tools/HSDLib/HSDRawViewer/bin/Release/net6.0-windows/HSDRawViewer.exe"
POSE = ROOT / "utility/assets/vanilla/custom_poses/Bowser/blastoise.yml"
AJ = ROOT / "utility/assets/vanilla/Bowser/PlKpAJ.dat"
WORK = ROOT / "tmp_blast"
WORK.mkdir(exist_ok=True)

COSTUMES = [("PlKpBk", 0), ("PlKpBu", 1), ("PlKpRd", 2), ("PlKpOr", 3)]
