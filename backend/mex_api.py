...
@app.route('/api/mex/bundle/list', methods=['GET'])
def list_bundles():
...
@app.route('/api/mex/bundle/delete/<bundle_id>', methods=['POST'])
...
@app.route('/api/mex/bundle/update/<bundle_id>', methods=['POST'])
...
@app.route('/api/mex/bundle/update-image/<bundle_id>', methods=['POST'])
...
@app.route('/api/dolphin/iso-folder', methods=['GET'])
...
def run_bundle_export(export_id, name, description, build_name, vanilla_iso_path, exported_iso_path, texture_pack_path, image_path=None):
    ...
    try:
        ...
        # 4. Create .ssbm ZIP - use export_id as the bundle ID for storage
        bundle_id = export_id
        ...
        # 5. Also save image separately for display in UI (image is also in the bundle for portability)
        if image_path and Path(image_path).exists():
...
@app.route('/api/mex/bundle/export', methods=['POST'])
def export_bundle():
...
@app.route('/api/mex/bundle/download/<bundle_id>', methods=['GET'])
...
@app.route('/api/mex/bundle/install/<bundle_id>', methods=['POST'])
...
def run_bundle_import(import_id, bundle_path, slippi_path, vanilla_iso_path, delete_on_complete=True):
    ...
    Args:
        delete_on_complete: If True, delete the bundle file after processing (for uploaded temp files).
                           If False, keep the file (for bundles installed from storage).
    """
    try:
        ...
        # 5. Copy textures to Load folder
        ...
        # 6. Enable HiresTextures=True in GFX.ini
...
@app.route('/api/mex/bundle/import', methods=['POST'])
def import_bundle():
...
@app.route('/api/mex/bundle/preview', methods=['POST'])
def preview_bundle():
