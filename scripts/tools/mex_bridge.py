...
class MexManager:
    ...
    def export_iso(
        ...
        Args:
            ...
            skip_compression: Skip ApplyCompression entirely (for texture pack mode)

        Returns:
            Dict with export results
        """
        cmd = [str(self.cli_path), "export", str(self.project_path), str(output_path),
               str(csp_compression), str(use_color_smash).lower(), str(skip_compression).lower()]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python scripts/tools/mex_bridge.py info")
        print("  python scripts/tools/mex_bridge.py list")
        print("  python scripts/tools/mex_bridge.py import <fighter> <zip>")
        sys.exit(1)
