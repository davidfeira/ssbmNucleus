...
ARTIFACT_ROOT="/mnt/c/Users/david/projects/NucleusDesktop/tests/artifacts/wsl/controller-smoke"
...
DOLPHIN_EMU_USERPATH="$TEST_USER" timeout 90s \
...
for _ in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:8080/ >/dev/null; then
    RESPONSE="$(curl -fsS \
      ...
      -X POST \
      -d '{"buttons":{"a":true},"frames":6}' \
      http://127.0.0.1:8080/api/controller/0)"
    echo "$RESPONSE"
    if grep -q '"screenshot"' <<<"$RESPONSE"; then
      echo "controller script ok"
      exit 0
    fi
    break
  fi
