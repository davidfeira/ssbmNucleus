...
export default function FirstRunSetup({ onComplete, onCancel = null }) {
  ...
  useEffect(() => {
    ...
    newSocket.on('setup_progress', (data) => {
      ...
      if (data.phase === 'extracting') {
        setStep('extracting')
      } else if (data.phase === 'copying_characters' || data.phase === 'copying_stages' || data.phase === 'copying_csp_data' || data.phase === 'extracting_sounds' || data.phase === 'seeding_builtins') {
        setStep('copying')
  ...
  const renderExtracting = () => (
    <div className="setup-step">
      <HexagonLoader
        size={104}
        className="setup-loader"
        progress={progressPercentage}
        label="Extracting ISO"
      />
      <h2>Extracting ISO</h2>
      <p className="setup-description">
        {getProgressMessage(progress.message, 'Extracting game files... This may take 1-2 minutes.')}
      </p>
    </div>
  )
  ...
  const renderCopying = () => (
    <div className="setup-step">
      <HexagonLoader
        ...
        className="setup-loader"
        progress={progressPercentage}
        label="Copying assets"
      />
      <h2>Copying Assets</h2>
      <p className="setup-description">
        {getProgressMessage(progress.message, 'Copying character and stage assets...')}
      </p>
    </div>
  )
